...
function setPixelUnsafe(frame: VisionFrame, x: number, y: number, values: number[]): void {
...
function setPixel(frame: VisionFrame, x: number, y: number, values: number[]): void {
...
function drawThickPoint(frame: VisionFrame, x: number, y: number, values: number[], thickness: number): void {
...
export function drawPoint(frame: VisionFrame, x: number, y: number, color: DrawColor = { r: 255, g: 0, b: 0 }, thickness = 1): VisionFrame {
  const out = frame.clone();
  const values = colorFor(out, color);

  drawThickPoint(out, Math.round(x), Math.round(y), values, Math.max(1, thickness));
...
export function drawFilledCircle(frame: VisionFrame, cx: number, cy: number, radius: number, color: DrawColor = { r: 255, g: 0, b: 0 }): VisionFrame {
  const out = frame.clone();
  const values = colorFor(out, color);
