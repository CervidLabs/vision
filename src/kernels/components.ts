...
export function connectedComponents(frame: VisionFrame, options: ConnectedComponentsOptions = {}): ConnectedComponent[] {
...
export function filterComponents(components: ConnectedComponent[], options: ConnectedComponentsOptions = {}): ConnectedComponent[] {
  const minArea = options.minArea ?? 1;
  const maxArea = options.maxArea ?? Number.POSITIVE_INFINITY;
