...
interface Pending { resolve: (data: ArrayBuffer) => void; reject: (e: Error) => void; }
interface QueuedJob { id: number; job: CompressJob; pending: Pending; }
...
class PngCompressPool {
    ...
    run(job: CompressJob): Promise<ArrayBuffer> {
        if (this.closed) return Promise.reject(new Error('PngCompressPool is closed'));
